...
class GenericAssistant():
    ...
    def load_json_intents(self, intents):
        # cwd = os.getcwd()
        # intents_file = os.path.join(cwd, "training", intents)
    ...
    def train_model(self):
        ...
        sgd = SGD(learning_rate=0.0034, decay=1e-6, momentum=0.9, nesterov=True)
        self.model.compile(loss='categorical_crossentropy',
                           optimizer=sgd, metrics=['accuracy'])

        self.hist = self.model.fit(np.array(train_x), np.array(
            train_y), epochs=100, batch_size=5, verbose=1)
